...
**Answer** It narrows down the information from journalism courses to just the ID and the title, making it easier to parse through later. This way, we don't need to use the old variable with all of the columns. We also aren't altering the old data set and just creating a new one to use.
...
**Answer 41011 records, OR adds together all of the data that is either in the Theatre department or has more than 15 seats, AND only gets the data that matches both of these classifications**
...
**Answer: It says \<date\> not \<chr\>, R recognizes it as a date not a character now**
...
**Answer** It is categorizing each month by the first day of the month and putting all the data from that month under that date.
...
**Answer It is mutating the code and adding all the total votes for all of the candidates then another column called percent moore, which divides moore's votes by the total to see how much of the total votes he got in each county**
...
**Answer It organized the rows in the order by which county had the highest percentage of moore votes to the lowest**
...
**Answer It is interesting because it shows the counties with the lowest percentage of moore votes. It shows which counties voted less for him.**
...
**Answer str_to_lower(): converts a character column into all lowercase, str_to_title(): converts to title case where the first letter of each word is capitalized**
...
**Answer Yes, this shows the states in order of those with highest expenditures to lowest**
...
1.  I would use dates to narrow down in UMD courses the courses that are only offered for a limited time during the semester

2.  I would use mutate on UMD courses to create a new column with the location by building to figure out which buildings hold the most classes

3.  I want to try filtering how many UMD courses are part of Honors or Scholars programs from their titles
