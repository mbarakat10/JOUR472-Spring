...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer 50 rows**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer created a new column where all the zip codes are only five digits**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Does that zip code make sense to you? **21202, this makes sense because this zip code is based in Baltimore around the harbor area, a lot of tourist attractions are based in Baltimore as it is one of the biggest cities and gets a lot of tourists**
