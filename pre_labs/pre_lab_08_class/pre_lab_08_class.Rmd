...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer: first step is making new sheet to count calls by month, mutate a new column to pull month out of date as an abbreviated name, then group all the calls under the same month and count how many total calls were in each month, the last step is to arrange the data so the months with the highest number of calls is first**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: I see a bar chart with the months on the x-axis and the number of calls per each month on the y-axis. The months are in order from least to most calls from left to right.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: months on left, calls on bottom, ordered from top to bottom most calls to least**
...
**Task** Run the following code. What do you think this does? **Answer: saves a version of the chart as a png in the assignment folder**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: new sheet, grouped by each date, and counted total calls per each date**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: made a line chart with date on x-axis and total calls on y, each date has how many calls, dates ordered by time**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: put dates by weeklong calls, instead of for each day**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: put date labels at angle to better see**
