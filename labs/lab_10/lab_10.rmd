...
md_statewide_county_votes <- md_counties |> inner_join(md_statewide_county_votes, join_by(geoid))
```
...
ggplot() +
  geom_sf(data = md_statewide_county_votes, aes(fill=pct_diff)) +
  theme_minimal()
```
...
**A2. One county has a much higher rate than the others. Why is this? I would want to see which months have the highest rates.**
...
county_outages_march2023 <- county_outages_march2023 |>
  mutate(
    rate_per_10k = (outage_customers/total_customers)*10000
  )
```
...
county_outages_merge <- md_counties_fixed |> inner_join(county_outages_march2023, join_by(name==county))
```

```{r}
#Map it out
ggplot() +
  geom_sf(data = county_outages_merge, aes(fill=rate_per_10k)) +
  theme_minimal()
```
...
ev_counties <- md_counties_fixed |> inner_join(ev_2020_2023, join_by(name==county))
```
...
ev_counties <- md_county_population |> inner_join(ev_counties, join_by(geoid))
```

```{r}
#Calculate per capita rate for March 2023
ev_counties <- ev_counties |>
  mutate(
    per_capita = march_2023/estimate
  )
```
...
ggplot() +
  geom_sf(data = ev_counties, aes(fill=per_capita)) +
  theme_minimal()
```
