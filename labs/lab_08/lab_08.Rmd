...
```{r}
pg_county_overdose_calls <- read_csv('data/prince_georges_2022_overdoses.csv') |>
  clean_names() |>
  mutate(
    date = date(datetime),
    week = week(datetime),
...
Compare the results here to those from the pre_lab_08 bar chart - are there major differences in the months with the highest and lowest figures? Describe that below. **The month with the highest percent of total calls was December, but in the pre lab, it was August. The month with the lowest was February in the pre lab, but here it's January.**

**A1.**

```{r}
pg_county_month_percent <- pg_county_overdose_calls |>
  group_by(month) |>
  summarise(
    count_calls = n()
  ) |> 
  mutate(
    total_calls = sum(count_calls),
    pct_total = (count_calls/total_calls) * 100
  ) |>
...
```{r}
pg_county_month_percent |>
  ggplot() +
  geom_bar(aes(x=month, weight=pct_total)) + 
  coord_flip() + 
  labs(
    title = 'One in 10 Overdose Calls in Prince George\'s County happen in December',
    x = 'Month',
...
**A2. Week 28 in the year has the highest number of calls, which falls in July. The outlier – also the week with the lowest number of calls – only has 4 but it is the final week of the year, so that makes sense. The data peaks on the line chart in the middle of the year.**

```{r}
pg_county_week <- pg_county_overdose_calls |>
  group_by(week) |>
  summarise(
    total_calls = n()
  ) |>
...
pg_county_week |>
  ...
  labs(
    title = 'PG County Overdose Calls by Week',
    x = 'Week',
    y = 'Total Calls',
    caption = 'Source: PG County EMS'
  )
...
**A3. In general, as median income increases, the total number of overdose calls decreases. At the lower median incomes though, the total calls are spread out evenly.**

```{r}
pg_calls_zipcodes <- pg_county_overdose_calls |>
  group_by(zipcode) |>
  summarise(
    total_calls = n()
  ) 
...
pg_calls_with_income |>
  ...
  labs(
    title = 'PG County Overdose Calls by Median Income',
    x = 'Median Income',
    y = 'Total Calls',
    caption = 'Source: PG County EMS, Census Bureau'
  ) +
