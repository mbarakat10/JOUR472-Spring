...
**A1.** <https://datawrapper.dwcdn.net/KkhrA/2/>

**I decided to make a grouped bar chart so readers can compare how each
candidate did in each county and see the percentages side-by-side. The
headline is the most newsworthy aspect, which is that Hogan received a
higher percentage of votes in each county, and the description describes
Hogan's highest.**
...
md_elections_join <- md_gov_county |>
  inner_join(md_gov_county_18, by=c('geoid','county')) |>
  mutate(
    cox_pct = cox/(moore+cox+lashar+harding+wallace+sefcik+write_ins.x)*100,
    hogan_pct = hogan/(hogan+jealous+quinn+schlakman+write_ins.y)*100
         )
  
write_csv(md_elections_join, "data/md_elections_join.csv")
```
...
**A2.** <https://datawrapper.dwcdn.net/qoitf/1/>

**The story is the spike in mail-in votes in 2020 (most likely due to
COVID) and then the immediate decline again in 2022**
...
write_csv(md_votes_by_type, "data/md_votes_by_type_pcts.csv")
```
...
**A3.** <https://datawrapper.dwcdn.net/cMbIf/1/>

**The story here is why this county alone has almost one-tenth of the
calls in the county. I want to know the populations of each zip code
because I think that will impact the significance of the number of
calls.**
...
montgomery_2022_overdoses_zip <- montgomery_2022_overdoses |>
  ...
  mutate(
    ...
    pct_total = (total_calls/total)*100
  )

write_csv(montgomery_2022_overdoses_zip, "data/moco_2022_overdoses_zip_2.csv")
