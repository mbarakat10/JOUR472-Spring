...
```{r}
and_climate_seats_1 <- umd_courses |> 
...
**Q2.** Of the departments listed in A2, which one is the most interesting to you, and why? **A2. University Honors, It's interesting that the honors program offers that many courses on climate and has the third highest number of classes on climate issues**
...
**A3. Business and Management has the most courses in the Fall 2023 term, English is no longer in the top 5 departments after filtering by the departments with at least one syllabus posted**

```{r}
fall_term <- umd_courses |>
...
**Q6.** Choose your own adventure: using either the UMD courses or campaign expenses data, ask and answer a question that is different than the previous ones. You can use one of the ones you provided in the pre_lab. **A6.** I want to try filtering how many UMD courses are part of Honors programs or have Honors in the titles from their titles, 1004 is the answer

```{r}
honors_courses <- umd_courses |> filter(str_detect(title, "Honors"))
